...
function wwgcbar_scripts() {
    ...
    $plugin_version = get_file_data(__FILE__, array('Version' => 'Version'))['Version'] ?? '1.0.0';

    wp_register_style('wwgcbar_main_style', plugins_url() . '/' . $pluginName . '/includes/stylesheets/ww-gdpr-screen.css', array(), $plugin_version, 'all');
    wp_enqueue_style('wwgcbar_main_style');

    wp_register_script('wwgcbar_main_script', plugins_url() . '/' . $pluginName . '/includes/js/ww-gdpr-scripts.js', array('jquery'), $plugin_version, true);
...
if (is_admin()) {
    function wwgcbar_admin_scripts() {
        ...
        wp_register_style('wwgcbar_admin_style', plugins_url() . '/' . $pluginName . '/includes/stylesheets/ww-gdpr-admin-screen.css', array(), $plugin_version, 'all');
        wp_enqueue_style('wwgcbar_admin_style');

        wp_register_script('wwgcbar_admin_scripts', plugins_url() . '/' . $pluginName . '/includes/js/ww-gdpr-admin-scripts.js', array('jquery'), $plugin_version, true);
...
function wwgcbar_shortcode($args) {
    ...
    $args = shortcode_atts(array(), $args, 'wwgcbar');
